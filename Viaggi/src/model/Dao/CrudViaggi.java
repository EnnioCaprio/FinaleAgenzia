...
public class CrudViaggi extends Connessione implements ICrudViaggi 
{
	...
	public Viaggi controlloVuoto(Viaggi v) {
		try {
			...
			while(rs.next()) {
				int quantitàPa = rs.getInt(7);
				if(quantitàPa == 0) {
					String query = "update viaggi set disponibilità = ? where quantita = ?";
					ps = conn.prepareStatement(query);
					ps.setInt(1, 0);
					ps.setInt(2, quantitàPa);
				}
			}
			ps.executeUpdate();
			ps.close();
	...
	@Override
	public Viaggi ritornaUno(Viaggi v) {
		try {
			conn = connetti();
			...
			rs = s.executeQuery(query);
			while(rs.next()) {
				int quantità = rs.getInt(7);
				if(quantità > 0) {
					String queryU = "update viaggi set disponibilità = ? where quantita = ?";
					ps = conn.prepareStatement(queryU);
					ps.setInt(1, 1);
					ps.setInt(2, v.getQuantità());
					ps.executeUpdate();
				}
			}
		} catch (SQLException e) {
